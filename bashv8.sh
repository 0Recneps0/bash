...
# --- FZF & Fuzzy Search Section ---
# All fzf, fuzzy search, and quick guide functions, aliases, and settings are consolidated here.
...
# Fuzzy Search Quick Guide
fzf_search_quick_guide() {
...
forensic_checks() {
    ...
    echo
    color_echo "${CYAN}Decrypt a GPG-encrypted file (if found during forensics)? [y/N]${NC}"
    local decrypt_choice="n"
    read -rp "Decrypt file? [y/N]: " -n 1 decrypt_choice
    echo
...
# Enhanced run all features with progress tracking
run_all_features() {
...
feature_menu() {
    ...
    echo "  14. 🔑 Decrypt GPG/Encoded File"
    echo "  15. 🚪 Exit"
    echo
}
...
# Decrypt a GPG-encrypted file interactively
forensics_decrypt_message() {
    color_echo "${CYAN}Enter the path to the encrypted or encoded file (.gpg, .asc, .txt, etc):${NC}"
    local input_file output_file search_dir
    set +e  # Disable exit on error for this function
    if command_exists fzf && [[ -t 0 ]]; then
        color_echo "${BLUE}Tip: Use fuzzy search to select a file. Leave blank to search from root directory.${NC}"
        color_echo "${CYAN}Enter directory to search for files (leave blank for root directory):${NC}"
        read -rp "Directory: " search_dir
        if [[ -z "$search_dir" ]]; then
            search_dir="/"
        fi
        if [[ -d "$search_dir" ]]; then
            input_file=$(find "$search_dir" -type f 2>/dev/null | fzf --prompt="Select file: " --height=20 --border --ansi)
            color_echo "Selected file: $input_file"
        fi
        if [[ -z "$input_file" ]]; then
            color_echo "${RED}No file selected from fzf."
            read -rp "File: " input_file
        fi
    else
        read -rp "File: " input_file
    fi
    if [[ ! -f "$input_file" ]]; then
        color_echo "${RED}File not found: $input_file${NC}"
        ls -l "$input_file" 2>&1
        set -e
        return 1
    fi
    color_echo "${CYAN}Enter output file path (leave blank to print to screen):${NC}"
    read -rp "Output file: " output_file
    color_echo "Attempting GPG decryption: gpg --quiet --batch --yes --decrypt '$input_file'"
    gpg --quiet --batch --yes --decrypt "$input_file" 2> /tmp/gpg_error.log > /tmp/forensics_decrypt_out
    local gpg_status=$?
    if [[ $gpg_status -eq 0 ]]; then
        if [[ -z "$output_file" ]]; then
            color_echo "${YELLOW}GPG decryption successful. Output below:${NC}"
            cat /tmp/forensics_decrypt_out
        else
            mv /tmp/forensics_decrypt_out "$output_file"
            color_echo "${GREEN}Decryption complete. Output saved to: $output_file${NC}"
        fi
        rm -f /tmp/forensics_decrypt_out /tmp/gpg_error.log
        set -e
        return 0
    else
        color_echo "${RED}GPG decryption failed. Error output below:${NC}"
        cat /tmp/gpg_error.log
        color_echo "File debug info:"
        ls -l "$input_file" 2>&1
        file "$input_file" 2>&1
        head -20 "$input_file" 2>&1
    fi
    rm -f /tmp/forensics_decrypt_out /tmp/gpg_error.log
    set -e

    # Try base64 decode
    if base64 --decode "$input_file" 2>/dev/null > /tmp/forensics_decrypt_out; then
        if [[ $(file -b --mime-type /tmp/forensics_decrypt_out) =~ text|xml|json ]]; then
            if [[ -z "$output_file" ]]; then
                color_echo "${YELLOW}Base64 decode successful. Output below:${NC}"
                cat /tmp/forensics_decrypt_out
            else
                mv /tmp/forensics_decrypt_out "$output_file"
                color_echo "${GREEN}Base64 decode complete. Output saved to: $output_file${NC}"
            fi
            rm -f /tmp/forensics_decrypt_out
            return 0
        fi
        rm -f /tmp/forensics_decrypt_out
    fi
    ...
    # Try hex decode
    if xxd -r -p "$input_file" 2>/dev/null > /tmp/forensics_decrypt_out; then
        if [[ $(file -b --mime-type /tmp/forensics_decrypt_out) =~ text|xml|json ]]; then
            if [[ -z "$output_file" ]]; then
                color_echo "${YELLOW}Hex decode successful. Output below:${NC}"
                cat /tmp/forensics_decrypt_out
            else
                mv /tmp/forensics_decrypt_out "$output_file"
                color_echo "${GREEN}Hex decode complete. Output saved to: $output_file${NC}"
            fi
            rm -f /tmp/forensics_decrypt_out
            return 0
        fi
    ...
    if command_exists tr; then
        if tr 'A-Za-z' 'N-ZA-Mn-za-m' < "$input_file" > /tmp/forensics_decrypt_out; then
            if [[ $(file -b --mime-type /tmp/forensics_decrypt_out) =~ text|xml|json ]]; then
                if [[ -z "$output_file" ]]; then
                    color_echo "${YELLOW}ROT13 decode successful. Output below:${NC}"
                    cat /tmp/forensics_decrypt_out
                else
                    mv /tmp/forensics_decrypt_out "$output_file"
                    color_echo "${GREEN}ROT13 decode complete. Output saved to: $output_file${NC}"
                fi
                rm -f /tmp/forensics_decrypt_out
                return 0
            fi
    ...
    # Try gzip decompress
    if gzip -cd "$input_file" 2>/dev/null > /tmp/forensics_decrypt_out; then
        if [[ $(file -b --mime-type /tmp/forensics_decrypt_out) =~ text|xml|json ]]; then
            if [[ -z "$output_file" ]]; then
                color_echo "${YELLOW}Gzip decompress successful. Output below:${NC}"
                cat /tmp/forensics_decrypt_out
            else
                mv /tmp/forensics_decrypt_out "$output_file"
                color_echo "${GREEN}Gzip decompress complete. Output saved to: $output_file${NC}"
            fi
            rm -f /tmp/forensics_decrypt_out
            return 0
        fi
    ...
    if bzip2 -cd "$input_file" 2>/dev/null > /tmp/forensics_decrypt_out; then
        if [[ $(file -b --mime-type /tmp/forensics_decrypt_out) =~ text|xml|json ]]; then
            if [[ -z "$output_file" ]]; then
                color_echo "${YELLOW}Bzip2 decompress successful. Output below:${NC}"
                cat /tmp/forensics_decrypt_out
            else
                mv /tmp/forensics_decrypt_out "$output_file"
                color_echo "${GREEN}Bzip2 decompress complete. Output saved to: $output_file${NC}"
            fi
            rm -f /tmp/forensics_decrypt_out
            return 0
        fi
    ...
    # Try xz decompress
    if xz -cd "$input_file" 2>/dev/null > /tmp/forensics_decrypt_out; then
        if [[ $(file -b --mime-type /tmp/forensics_decrypt_out) =~ text|xml|json ]]; then
            if [[ -z "$output_file" ]]; then
                color_echo "${YELLOW}XZ decompress successful. Output below:${NC}"
                cat /tmp/forensics_decrypt_out
            else
                mv /tmp/forensics_decrypt_out "$output_file"
                color_echo "${GREEN}XZ decompress complete. Output saved to: $output_file${NC}"
            fi
            rm -f /tmp/forensics_decrypt_out
            return 0
        fi
    ...
    # Try unzip (if zip file)
    if unzip -p "$input_file" 2>/dev/null > /tmp/forensics_decrypt_out; then
        if [[ $(file -b --mime-type /tmp/forensics_decrypt_out) =~ text|xml|json ]]; then
            if [[ -z "$output_file" ]]; then
                color_echo "${YELLOW}ZIP decompress successful. Output below:${NC}"
                cat /tmp/forensics_decrypt_out
            else
                mv /tmp/forensics_decrypt_out "$output_file"
                color_echo "${GREEN}ZIP decompress complete. Output saved to: $output_file${NC}"
            fi
            rm -f /tmp/forensics_decrypt_out
            return 0
        fi
    ...
    color_echo "${RED}Could not decrypt or decode the file. Supported: GPG, base64, hex, ROT13, gzip, bzip2, xz, zip.${NC}"
    rm -f /tmp/forensics_decrypt_out
    return 1
}
    ...
    while true; do
        ...
        case $choice in
            ...
            15)
                ...
                ;;
            *)
                color_echo "${RED}Invalid option. Please choose 1-15.${NC}" >&2
                ;;
        esac
